...
from photo_organizer.converter.core import batch_process, save_report
...
class TIFFConverterGUI:
    ...
    def _create_widgets(self):
        ...
        # Directory Selection
        dir_frame = ttk.LabelFrame(container, text="Source Directory", padding=10)
        ...
        # Options
        opt_frame = ttk.LabelFrame(container, text="Conversion Options", padding=10)
    ...
    def _run_conversion(self):
        try:
            ...
            tiff_files = list(self.source_dir.glob("*.tif")) + \
                        list(self.source_dir.glob("*.tiff")) + \
                        list(self.source_dir.glob("*.TIF")) + \
                        list(self.source_dir.glob("*.TIFF"))
            ...
            self.root.after(0, lambda: self.log(f"Found {len(tiff_files)} TIFF files"))
            ...
            self.root.after(0, lambda: self.log(f"Report saved to: {report_path}"))
            
            self.root.after(0, lambda: messagebox.showinfo(
                "Complete",
                f"Successfully processed {len(tiff_files)} files.\n"
                f"Report saved to conversion_report.json"
            ))
