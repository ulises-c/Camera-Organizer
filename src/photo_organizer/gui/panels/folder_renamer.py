...
from photo_organizer.renamer.engine import (
...
class FolderRenamerPanel(QWidget):
    """Collect folder-renamer options and display engine results."""

    def __init__(self, engine: Callable[..., Any] = process_folder_rename, parent=None):
        ...
        title = QLabel("📁 Folder Renamer")
        title.setStyleSheet("font-size: 18px; font-weight: 600;")
        layout.addWidget(title)
        layout.addWidget(
            QLabel("Rename NNNYMMDD camera folders to dates derived from photo metadata.")
        )

        form = QFormLayout()
        self.source_edit = QLineEdit()
        self.source_edit.setPlaceholderText("Choose a parent folder…")
        ...
        form.addRow("Parent folder", source_row)
        layout.addLayout(form)

        self.recursive_checkbox = QCheckBox("Search subfolders recursively")
        self.recursive_checkbox.setChecked(FOLDER_DEFAULT_OPTIONS["recursive"])
        self.include_model_checkbox = QCheckBox("Include camera model in folder names")
        self.include_model_checkbox.setChecked(FOLDER_DEFAULT_OPTIONS["include_model"])
        self.merge_checkbox = QCheckBox("Merge into existing destination folders")
        self.merge_checkbox.setChecked(FOLDER_DEFAULT_OPTIONS["merge"])
        self.merge_help_label = QLabel(
            "Merges never overwrite files; name conflicts receive a _dupN suffix."
        )
        self.merge_help_label.setWordWrap(True)
        layout.addWidget(self.recursive_checkbox)
        layout.addWidget(self.include_model_checkbox)
        layout.addWidget(self.merge_checkbox)
        layout.addWidget(self.merge_help_label)

        self.dry_run_checkbox = QCheckBox("Preview only")
        self.dry_run_checkbox.setChecked(FOLDER_DEFAULT_OPTIONS["dry_run"])
        ...
        self.status_label = QLabel("Choose a parent folder to preview renames.")
    ...
    def _browse(self) -> None:
        selected = QFileDialog.getExistingDirectory(
            self, "Choose parent folder", self.source_edit.text()
        )
        if selected:
    ...
    def build_options(self) -> dict:
        ...
        return {
            ...
            "include_model": self.include_model_checkbox.isChecked(),
    ...
    def _update_mode_copy(self, dry_run: bool) -> None:
        if dry_run:
            ...
            self.safety_label.setText("Live mode — folders may be renamed or merged.")
            self.run_button.setText("Rename folders")

    def _start(self) -> None:
        if not self.run_button.isEnabled():
            return
        if not self.dry_run_checkbox.isChecked():
            message = "Rename the matching folders now? This changes names on disk."
            if self.merge_checkbox.isChecked():
                message += " Existing destination folders may be merged."
            answer = QMessageBox.question(
                self,
                "Confirm live folder rename",
                message,
    ...
    def _set_inputs_enabled(self, enabled: bool) -> None:
        ...
        self.include_model_checkbox.setEnabled(enabled)
        self.merge_checkbox.setEnabled(enabled)
